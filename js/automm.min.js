...
prevNote=that.model.pattern[i%that.model.octaveNotes];}};that.draw=function(){that.setup();that.d3container=d3.select("#"+that.container.attr('id')).select('#piano');var svg=that.d3container.append("svg");svg.attr("style","height: 100%;");svg.attr("viewBox",that.model.viewbox.dim);svg.attr("role","application");svg.attr("focusable",true);svg.attr("tabindex","0");svg.attr("aria-labelledby","ariaTitle");that.noteGroup=svg.append("g");that.noteGroup.attr("transform","translate("+that.model.padding/2+","+that.model.padding/2+")");that.noteGroup.attr("role","controlgroup");that.noteGroup.attr("id","noteGroup");that.noteGroup.attr("focusable",true);that.render();};that.update=function(param,value){that.applier.requestChange(param,value);that.container.children("#piano").empty();that.draw();that.events.afterUpdate.fire();};that.sendNoteCalc=function(){that.events.afterNoteCalc.fire(that.model.keys);};};automm.piano.postInitFunction=function(that){var pianoElements=that.container.find("#piano").length;if(that.model.auto&&pianoElements<1){that.container.append("<div id='piano'></div>");pianoElements=1;}
...
for(i=0;i<that.model.keys.black.notes.length;i+=1){noteNum=that.model.keys.black.notes[i];notePos.width=that.calcNoteDim(that.model.keys.black,noteNum,"width");notePos.height=that.calcNoteDim(that.model.keys.black,noteNum,"height");that.drawNote(that.model.keys.black,notePos.width,notePos.height,noteNum);}};that.draw=function(){that.setup();that.d3container=d3.select("#"+that.container.attr('id')).select('#grid');var svg=that.d3container.append("svg");svg.attr("style","height: 100%;");svg.attr("viewBox",that.model.viewbox.dim);svg.attr("role","application");svg.attr("focusable",true);svg.attr("tabindex","0");svg.attr("aria-labelledby","ariaTitle");that.noteGroup=svg.append("g");that.noteGroup.attr("transform","translate("+that.model.padding/2+","+that.model.padding/2+")");that.noteGroup.attr("role","controlgroup");that.noteGroup.attr("id","noteGroup");that.noteGroup.attr("focusable",true);that.render();};that.update=function(param,value){that.applier.requestChange(param,value);that.container.children("#grid").empty();that.draw();that.events.afterUpdate.fire();};that.sendNoteCalc=function(){that.events.afterNoteCalc.fire(that.model.keys);};};automm.grid.postInitFunction=function(that){var gridElements=that.container.find("#grid").length;if(that.model.auto&&gridElements<1){that.container.append("<div id='grid'></div>");gridElements=1;}
...
lastClicked=note;});});};that.onNote=function(note){if(that.model.isShift){that.polyNotes[that.polyNotes.length]=note;}};that.afterShift=function(){fluid.each(that.polyNotes,function(note){that.events.afterNote.fire(note);});that.polyNotes=[];};};automm.eventBinder.postInitFunction=function(that){that.bindEvents();that.events.afterUpdate.addListener(that.bindEvents);that.events.onNote.addListener(that.onNote);};}(jQuery));