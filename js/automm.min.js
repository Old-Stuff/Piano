...
lastClicked=note;});});};that.onNote=function(note){if(that.model.isShift){that.polyNotes[that.polyNotes.length]=note;}};that.afterShift=function(){fluid.each(that.polyNotes,function(note){that.events.afterNote.fire(note);});};};automm.eventBinder.postInitFunction=function(that){that.bindEvents();that.events.afterUpdate.addListener(that.bindEvents);that.events.onNote.addListener(that.onNote);};}(jQuery));