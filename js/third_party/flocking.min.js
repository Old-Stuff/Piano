...
return resultMap;};flock.input.set=function(root,path,val,baseTarget,valueParser){return typeof(path)==="string"?flock.input.setValueForPath(root,path,val,baseTarget,valueParser):flock.input.setValuesForPaths(root,path,baseTarget,valueParser);};fluid.defaults("flock.nodeList",{gradeNames:["fluid.littleComponent","autoInit"],members:{nodes:[]}});flock.nodeList.preInit=function(that){that.head=function(node){that.nodes.unshift(node);};that.before=function(refNode,node){var refIdx=that.nodes.indexOf(refNode);that.at(refIdx,node);};that.after=function(refNode,node){var refIdx=that.nodes.indexOf(refNode);that.at(refIdx+1,node);};that.at=function(idx,node){that.nodes.splice(idx,0,node);};that.tail=function(node){that.nodes.push(node);};that.remove=function(node){var idx=that.nodes.indexOf(node);that.nodes.splice(idx,1);};};fluid.defaults("flock.enviro",{gradeNames:["fluid.modelComponent","flock.nodeList","autoInit"],model:{playState:{written:0,total:null},isPlaying:false},audioSettings:{rates:{audio:44100,control:64,constant:1},chans:2,numBuses:2,bufferSize:(flock.platform.os==="Win32"&&flock.platform.browser.mozilla)?16384:4096,genPollIntervalFactor:flock.platform.isLinuxBased?1:20},components:{asyncScheduler:{type:"flock.scheduler.async"},audioStrategy:{type:"flock.enviro.audioStrategy",options:{audioSettings:"{enviro}.options.audioSettings",model:"{enviro}.model"}}}});flock.enviro.preInit=function(that){that.audioSettings=that.options.audioSettings;that.buses=flock.enviro.createAudioBuffers(that.audioSettings.numBuses,that.audioSettings.rates.control);that.buffers={};that.play=function(dur){var playState=that.model.playState,sps=dur*(that.audioSettings.rates.audio*that.audioSettings.chans);playState.total=dur===undefined?Infinity:playState.total===Infinity?sps:playState.written+sps;that.audioStrategy.startGeneratingSamples();that.model.isPlaying=true;};that.stop=function(){that.audioStrategy.stopGeneratingSamples();that.model.isPlaying=false;};that.reset=function(){that.stop();that.asyncScheduler.clearAll();while(that.nodes.length>0){that.nodes.pop();}};that.loadBuffer=function(name,src,onLoadFn){if(!src&&onLoadFn){onLoadFn(that.buffers[name],name);return;}
flock.audio.decode(src,function(decoded){var chans=decoded.data.channels;that.buffers[name]=chans;if(onLoadFn){onLoadFn(chans,name);}});};};flock.enviro.finalInit=function(that){that.gen=that.audioStrategy.nodeEvaluator.gen;};flock.enviro.createAudioBuffers=function(numBufs,kr){var bufs=[],i;for(i=0;i<numBufs;i++){bufs[i]=new Float32Array(kr);}
return bufs;};fluid.defaults("flock.enviro.audioStrategy",{gradeNames:["fluid.modelComponent"],components:{nodeEvaluator:{type:"flock.enviro.nodeEvaluator",options:{numBuses:"{enviro}.options.audioSettings.numBuses",controlRate:"{enviro}.options.audioSettings.rates.control",buses:"{enviro}.buses",nodes:"{enviro}.nodes"}}}});fluid.defaults("flock.enviro.nodeEvaluator",{gradeNames:["fluid.littleComponent","autoInit"],mergePolicy:{nodes:"nomerge",buses:"nomerge"}});flock.enviro.nodeEvaluator.finalInit=function(that){that.nodes=that.options.nodes;that.buses=that.options.buses;that.gen=function(){var numBuses=that.options.numBuses,busLen=that.options.controlRate,i,bus,j,node;for(i=0;i<numBuses;i++){bus=that.buses[i];for(j=0;j<busLen;j++){bus[j]=0;}}
for(i=0;i<that.nodes.length;i++){node=that.nodes[i];node.gen(node.model.blockSize);}};};fluid.defaults("flock.autoEnviro",{gradeNames:["fluid.littleComponent","autoInit"]});flock.autoEnviro.finalInit=function(that){if(!flock.enviro.shared){flock.init();}};flock.synth=function(def,options){var that=fluid.initComponent("flock.synth",options);that.rate=flock.rates.AUDIO;that.enviro=flock.enviro.shared;that.model.blockSize=that.enviro.audioSettings.rates.control;that.model.synthDef=def;that.gen=function(){var nodes=that.ugens.active,i,node;for(i=0;i<nodes.length;i++){node=nodes[i];node.gen(node.model.blockSize);}};that.get=function(path){return flock.input.get(that.ugens.named,path);};that.set=function(path,val,swap){return flock.input.set(that.ugens.named,path,val,undefined,function(ugenDef,path,target,previous){var ugen=flock.parse.ugenDef(ugenDef,{audioSettings:that.enviro.audioSettings,buses:that.enviro.buses,buffers:that.enviro.buffers});that.ugens.replace(ugen,previous,swap);return ugen;});};that.input=function(path,val,swap){return!path?undefined:typeof(path)==="string"?arguments.length<2?that.get(path):that.set(path,val,swap):flock.isIterable(path)?that.get(path):that.set(path,val,swap);};that.play=function(){var e=that.enviro;if(e.nodes.indexOf(that)===-1){e.head(that);}
if(!e.isPlaying){e.play();}};that.pause=function(){that.enviro.remove(that);};that.init=function(){that.out=flock.parse.synthDef(that.model.synthDef,{visitors:that.ugens.add,buffers:that.enviro.buffers,buses:that.enviro.buses,audioSettings:that.enviro.audioSettings});if(that.options.addToEnvironment!==false){that.enviro.tail(that);}};that.init();return that;};fluid.defaults("flock.synth",{gradeNames:["fluid.modelComponent","flock.autoEnviro"],argumentMap:{options:1},components:{ugens:{type:"flock.synth.ugenCache"}}});fluid.defaults("flock.synth.ugenCache",{gradeNames:["fluid.littleComponent","autoInit"]});flock.synth.ugenCache.finalInit=function(that){that.named={};that.active=[];that.add=function(ugens){var i,ugen;ugens=fluid.makeArray(ugens);for(i=0;i<ugens.length;i++){ugen=ugens[i];if(ugen.gen){that.active.push(ugen);}
...
return that;};flock.synth.polyphonic=function(def,options){var that=fluid.initComponent("flock.synth.polyphonic",options);that.model.synthDef=def;that.activeVoices={};that.freeVoices=[];that.noteChange=function(voice,eventName,changeSpec){var noteEventSpec=that.options.noteSpecs[eventName];changeSpec=$.extend({},noteEventSpec,changeSpec);voice.input(changeSpec);};that.noteOn=function(noteName,changeSpec){var voice=that.nextFreeVoice();if(that.activeVoices[noteName]){that.noteOff(noteName);}
...
that.nextFreeVoice=that.pooledVoiceAllocator;}else{that.nextFreeVoice=that.lazyVoiceAllocator;}};that.init();return that;};fluid.defaults("flock.synth.polyphonic",{gradeNames:["flock.synth.group"],argumentMap:{options:1},noteSpecs:{on:{"env.gate":1},off:{"env.gate":0}},maxVoices:16,initVoicesLazily:true,amplitudeKey:"env.sustain",amplitudeNormalizer:"static"});}());
...
that.scheduleClock.clearAll();for(var listener in that.scheduleListeners){that.clear(listener);}};that.end=function(){that.intervalClock.end();that.scheduleClock.end();};};flock.scheduler.async.beat=function(bpm,options){var that=fluid.initComponent("flock.scheduler.async.beat",options);if(bpm!==undefined){that.timeConverter.options.bpm=bpm;}
...
that.context=flock.enviro.webAudio.audioContext;that.source=that.context.createBufferSource();that.jsNode=that.context.createJavaScriptNode(settings.bufferSize);that.source.connect(that.jsNode);};that.init();};flock.enviro.webAudio.contextConstructor=window.AudioContext||window.webkitAudioContext;fluid.demands("flock.enviro.audioStrategy","flock.platform.webAudio",{funcName:"flock.enviro.webAudio"});}());
var fluid=fluid||require("infusion"),flock=fluid.registerNamespace("flock");(function(){"use strict";var $=fluid.registerNamespace("jQuery");fluid.registerNamespace("flock.parse");flock.parse.synthDef=function(ugenDef,options){if(typeof(ugenDef.length)==="number"||(ugenDef.id!==flock.OUT_UGEN_ID&&ugenDef.ugen!=="flock.ugen.out")){ugenDef={id:flock.OUT_UGEN_ID,ugen:"flock.ugen.out",inputs:{sources:ugenDef,bus:0,expand:options.audioSettings.chans}};}
...
throw new Error("Invalid value type found in ugen definition.");};flock.parse.rateMap={"ar":flock.rates.AUDIO,"kr":flock.rates.CONTROL,"cr":flock.rates.CONSTANT};flock.parse.expandRate=function(ugenDef){ugenDef.rate=flock.parse.rateMap[ugenDef.rate]||ugenDef.rate;return ugenDef;};flock.parse.ugenDef=function(ugenDefs,options){var parseFn=flock.isIterable(ugenDefs)?flock.parse.ugensForDefs:flock.parse.ugenForDef;var parsed=parseFn(ugenDefs,options);return parsed;};flock.parse.ugensForDefs=function(ugenDefs,options){var parsed=[],i;for(i=0;i<ugenDefs.length;i++){parsed[i]=flock.parse.ugenForDef(ugenDefs[i],options);}
...
that.mulAdd(numSamps);};that.onInputChanged=function(inputName){var m=that.model,inputs=that.inputs;if(m.bufDef!==inputs.buffer||inputName==="buffer"){flock.buffer.resolveBufferDef(that);}
