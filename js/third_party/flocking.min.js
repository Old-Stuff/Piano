...
var flock=flock||{};(function(){"use strict";flock.OUT_UGEN_ID="flocking-out";flock.TWOPI=2.0*Math.PI;flock.LOG1=Math.log(0.1);flock.LOG001=Math.log(0.001);flock.rates={AUDIO:"audio",CONTROL:"control",CONSTANT:"constant"};flock.defaults=function(name,defaults){if(defaults){flock.defaults.store[name]=defaults;return defaults;}
return flock.defaults.store[name];};flock.defaults.store={};flock.defaults("flock.audioSettings",{rates:{audio:44100,control:64,constant:1},tableSize:8192,bufferSize:1024});flock.idIdx=0;flock.id=function(){return"flock-id-"+flock.idIdx++;};flock.identity=function(val){return val;};flock.isIterable=function(o){var l=o.length;return o&&l!==undefined&&typeof(l)==="number";};flock.generate=function(bufOrSize,generator){var buf=typeof(bufOrSize)==="number"?new Float32Array(bufOrSize):bufOrSize,i;if(typeof(generator)==="number"){var value=generator;generator=function(){return value;};}
...
var key=arrayChoose(collection.keys,strategy);return collection[key];};flock.normalize=function(buffer){var maxVal=0.0,i,current;for(i=0;i<buffer.length;i++){current=buffer[i];if(current>maxVal){maxVal=current;}}
if(maxVal>0.0){for(i=0;i<buffer.length;i++){buffer[i]/=maxVal;}}
return buffer;};flock.pathParseError=function(path,token){throw new Error("Error parsing path: "+path+". Segment '"+token+"' could not be resolved.");};flock.get=function(path,root){root=root||window;var tokenized=path===""?[]:String(path).split("."),valForSeg=root[tokenized[0]],i;for(i=1;i<tokenized.length;i++){if(valForSeg===null||valForSeg===undefined){flock.pathParseError(path,tokenized[i-1]);}
valForSeg=valForSeg[tokenized[i]];}
return valForSeg;};flock.invoke=function(path,args,root){var fn=flock.get(path,root);if(typeof(fn)!=="function"){throw new Error("Path '"+path+"' does not resolve to a function.");}
return fn.apply(null,args);};flock.shim={BlobBuilder:window.BlobBuilder||window.MozBlobBuilder||window.WebKitBlobBuilder||window.MSBlobBuilder||window.OBlobBuilder,URL:window.URL||window.webkitURL||window.msURL||window.oURL};flock.worker=function(code){var type=typeof(code),url,builder;if(type==="function"){code="("+code.toString()+")();";}else if(type!=="string"){throw Error("A flock.worker must be initialized with a String or a Function.");}
if(flock.shim.BlobBuilder){builder=new flock.shim.BlobBuilder();builder.append(code);url=flock.shim.URL.createObjectURL(builder.getBlob());}else{url="data:text/javascript;base64,"+window.btoa(code);}
return new Worker(url);};flock.worker.code={interval:function(){self.intervals={};self.onInterval=function(interval){self.postMessage({msg:"tick",value:interval});};self.schedule=function(interval){var id=setInterval(function(){self.onInterval(interval);},interval);self.intervals[interval]=id;};self.clear=function(interval){var id=self.intervals[interval];clearInterval(id);};self.clearAll=function(){for(var interval in self.intervals){self.clear(interval);}};self.addEventListener("message",function(e){self[e.data.msg](e.data.value);},false);}};flock.conductor=function(){var that={intervalWorker:flock.worker(flock.worker.code.interval)};that.schedulePeriodic=function(interval,fn){that.intervalWorker.addEventListener("message",function(e){if(e.data.value===interval){fn();}},false);that.intervalWorker.postMessage({msg:"schedule",value:interval});};that.clearPeriodic=function(interval){that.intervalWorker.postMessage({msg:"clear",value:interval});};that.clearAll=function(){that.intervalWorker.postMessage({msg:"clearAll"});};return that;};var setupEnviro=function(that){var setupFn=typeof(window.webkitAudioContext)!=="undefined"?flock.enviro.webkit:flock.enviro.moz;setupFn(that);};flock.enviro=function(options){options=options||{};var defaultSettings=flock.defaults("flock.audioSettings"),that={audioSettings:{rates:{audio:options.sampleRate||defaultSettings.rates.audio,control:options.controlRate||defaultSettings.rates.control,constant:options.constantRate||defaultSettings.rates.constant},chans:options.chans||2,bufferSize:options.bufferSize||defaultSettings.bufferSize,numBuses:options.numBuses||16},model:{playState:{written:0,total:null}},nodes:[],isPlaying:false};that.buses=flock.enviro.createAudioBuffers(that.audioSettings.numBuses,that.audioSettings.rates.control);that.buffers={};that.conductor=flock.conductor();that.play=function(dur){var playState=that.model.playState,sps=dur*(that.audioSettings.rates.audio*that.audioSettings.chans);playState.total=dur===undefined?Infinity:playState.total===Infinity?sps:playState.written+sps;that.startGeneratingSamples();that.isPlaying=true;};that.stop=function(){that.stopGeneratingSamples();that.isPlaying=false;};that.reset=function(){that.stop();that.conductor.clearAll();while(that.nodes.length>0){that.nodes.pop();}};that.gen=function(){that.clearBuses();flock.enviro.evalGraph(that.nodes,that.audioSettings.rates.control);};that.head=function(node){that.nodes.unshift(node);};that.before=function(refNode,node){var refIdx=that.nodes.indexOf(refNode);that.at(refIdx,node);};that.after=function(refNode,node){var refIdx=that.nodes.indexOf(refNode);that.at(refIdx+1,node);};that.at=function(idx,node){that.nodes.splice(idx,0,node);};that.tail=function(node){that.nodes.push(node);};that.remove=function(node){var idx=that.nodes.indexOf(node);that.nodes.splice(idx,1);};that.loadBuffer=function(name,src,onLoadFn){if(!src&&onLoadFn){onLoadFn(that.buffers[name],name);return;}
...
return outBuf;};flock.enviro.moz=function(that){var defaultSettings=flock.defaults("flock.audioSettings");that.audioEl=new Audio();that.model.writeInterval=1;that.model.sampleOverflow=0-(that.audioSettings.bufferSize*4);that.audioEl.mozSetup(that.audioSettings.chans,that.audioSettings.rates.audio);that.startGeneratingSamples=function(){if(that.scheduled){return;}
that.conductor.schedulePeriodic(that.model.writeInterval,that.writeSamples);that.scheduled=true;};that.writeSamples=function(){var playState=that.model.playState,currentOffset=that.audioEl.mozCurrentSampleOffset(),needed=currentOffset-playState.written,outBuf;if(needed<that.model.sampleOverflow||that.nodes.length<1){return;}
outBuf=new Float32Array(that.audioSettings.bufferSize*that.audioSettings.chans);flock.interleavedDemandWriter(outBuf,that.gen,that.buses,that.audioSettings);playState.written+=that.audioEl.mozWriteAudio(outBuf);if(playState.written>=playState.total){that.stop();}};that.stopGeneratingSamples=function(){that.conductor.clearPeriodic(that.model.writeInterval);that.scheduled=false;};};var setupWebKitEnviro=function(that){that.jsNode.onaudioprocess=function(e){var defaultSettings=flock.defaults("flock.audioSettings"),kr=defaultSettings.rates.control,playState=that.model,chans=that.audioSettings.chans,bufSize=that.audioSettings.bufferSize,numKRBufs=bufSize/kr,sourceBufs=that.buses,outBufs=e.outputBuffer,i,chan,samp;if(that.nodes.length<1){for(chan=0;chan<chans;chan++){flock.generate.silence(outBufs.getChannelData(chan));}
return;}
...
playState.written+=bufSize*chans;if(playState.written>=playState.total){that.stop();}};that.source.connect(that.jsNode);};flock.enviro.webkit=function(that){var defaultSettings=flock.defaults("flock.audioSettings");that.context=new webkitAudioContext();that.source=that.context.createBufferSource();that.jsNode=that.context.createJavaScriptNode(that.audioSettings.bufferSize);that.startGeneratingSamples=function(){that.jsNode.connect(that.context.destination);};that.stopGeneratingSamples=function(){that.jsNode.disconnect(0);};setupWebKitEnviro(that);};flock.enviro.shared=flock.enviro();flock.synth=function(def,options){var that={rate:flock.rates.AUDIO,enviro:flock.enviro.shared,ugens:flock.synth.ugenCache(),model:{synthDef:def},options:options||{}};that.gen=function(){flock.enviro.evalGraph(that.ugens.active,that.enviro.audioSettings.rates.control);};that.getUGenPath=function(path){var input=flock.get(path,that.ugens.named);return(input&&input.model&&typeof(input.model.value)!=="undefined")?input.model.value:input;};that.setUGenPath=function(path,val,swap){if(path.indexOf(".")===-1){throw new Error("Setting a ugen directly is not currently supported.");}
var lastSegIdx=path.lastIndexOf("."),ugenInputPath=path.substring(0,lastSegIdx),ugenPath=ugenInputPath.substring(0,ugenInputPath.lastIndexOf(".")),inputName=path.substring(lastSegIdx+1),ugen=flock.get(ugenPath,that.ugens.named),prevInputUGen=ugen.input(inputName),inputUGen=ugen.input(inputName,val);that.ugens.replace(inputUGen,prevInputUGen,swap);ugen.onInputChanged(inputName);return inputUGen;};that.input=function(path,val,swap){if(!path){return;}
var expanded=flock.synth.expandInputPath(path);return arguments.length<2?that.getUGenPath(expanded):that.setUGenPath(expanded,val,swap);};that.play=function(){var e=that.enviro;if(e.nodes.indexOf(that)===-1){e.head(that);}
if(!e.isPlaying){e.play();}};that.pause=function(){that.enviro.remove(that);};that.init=function(){var parseOptions=$.extend({},that.enviro.audioSettings,{visitors:that.ugens.add});that.out=flock.parse.synthDef(that.model.synthDef,parseOptions);if(that.options.addToEnvironment!==false){that.enviro.head(that);}};that.init();return that;};flock.synth.inputPathExpander=function(path){return path.replace(".",".inputs.");};flock.synth.expandInputPaths=function(paths){var expanded={};$.each(paths,function(path,value){var expandedPath=inputPathExpander(path);expanded[expandedPath]=value;});return expanded;};flock.synth.expandInputPath=function(path){return(typeof(path)==="string")?flock.synth.inputPathExpander(path):flock.synth.expandInputPaths(path);};flock.synth.ugenCache=function(){var that={named:{},active:[]};that.add=function(ugens){var i,ugen;ugens=$.makeArray(ugens);for(i=0;i<ugens.length;i++){ugen=ugens[i];if(ugen.gen){that.active.push(ugen);}
...
m.idx=bufIdx;};that.krSpeedGen=function(numSamps){var m=that.model,out=that.output,chan=that.inputs.channel.output[0],speedInc=1.0*that.inputs.speed.output[0],source=that.buffer,bufIdx=m.idx,bufLen=source.length,loop=that.inputs.loop.output[0],i;if(m.channel!==chan){m.channel=chan;that.buffer=source=flock.enviro.shared.buffers[m.name][chan];}
for(i=0;i<numSamps;i++){if(bufIdx>=bufLen){if(loop>0){bufIdx=0;}else{out[i]=0.0;continue;}}
...
throw new Error("Invalid value type found in ugen definition.");};flock.parse.rateMap={"ar":flock.rates.AUDIO,"kr":flock.rates.CONTROL,"cr":flock.rates.CONSTANT};flock.parse.expandRate=function(ugenDef){ugenDef.rate=flock.parse.rateMap[ugenDef.rate]||ugenDef.rate;return ugenDef;};flock.parse.ugensForDefs=function(ugenDefs,rates,visitors){var parsed=[],i;for(i=0;i<ugenDefs.length;i++){parsed[i]=flock.parse.ugenForDef(ugenDefs[i],rates,visitors);}
